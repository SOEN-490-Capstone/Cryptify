...
export function filterTransction(currencyType: CurrencyType, walletAddress: string,transactions: Transaction[], filters: string[]): Transaction[] {
    ...
    filters.map((filter) => {
        ...
        if(filtersByDate[1] === filter){
            transactions = transactions.filter((transaction) => new Date(dateToday.getFullYear(), dateToday.getMonth(), dateToday.getDay() - 90) <= transaction.createdAt); 
        }

        // filter for transactions that happened this current year
        if(filtersByDate[2] === filter){
            transactions = transactions.filter((transaction) => new Date().getFullYear() ===  new Date(transaction.createdAt).getFullYear()); 
        }

        // filter for transactions that happened this previous year
        if(filtersByDate[3] === filter){
            transactions = transactions.filter((transaction) => new Date().getFullYear() - 1 ===  new Date(transaction.createdAt).getFullYear()); 
        }

        // filter for transactions that are between two given dates
        if(filter.split("-").length == 2){
            const [fromDate, toDate] = filter.split("-");
            transactions = transactions.filter((transaction) =>  new Date(fromDate)  <= transaction.createdAt && new Date(toDate) >= transaction.createdAt); 
        }
    })
