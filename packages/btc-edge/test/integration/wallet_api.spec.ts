...
describe("Wallets", () => {
    ...
    describe("POST /users/:id/wallets", () => {
        ...
        it("should create the wallet and return the wallet with the balance", async () => {
            ...
            (fetch as jest.MockedFunction<typeof fetch>).mockResolvedValueOnce(new Response(JSON.stringify(outAndInTransactions))); 
    ...
    describe("DELETE /users/:id/wallets/:address", () => {
        it("should get all a users BTC wallets", async () => {
            ...
            const res = await agent(app.getHttpServer()).delete("/users/1/wallets/bc1qe4zsm2eeus8j7xqluprkud88wsrhrz8j9vlhqzdzq3e9eq4ygw8qazc3cn");
...
const outAndInTransactions = {
  ...
  txs: [
      ...
      inputs: [
            {
                  sequence: 4294967295,
          witness: '0247304402205fc54ff561950a63d26dc65b215470ee7c584b4ead95b4bd2a8f52be0a6afee502207d60b5cc4e106a280fd3ccd3c08132c0ad2f9217c78a3fd82fcd98bfc1c360800121023e743ef51388af2cca2ab59bd77fd48638e0a663a1959ced61ad46f6216b32d4',
          script: '',
          index: 0,
          prev_out: {
    ...
    },
    {
      ...
      inputs: [
            {
                  sequence: 4294967295,
          witness: '024730440220773c587448bf20226e923495c333711bcbd8708088650acce39439e8ff6f54fe022078c5e368d6ff96fed2d97e2bfd9a962d5aaebfa90a035f21e6ac9a9ec7eb05e50121034483675fadeaf959f2733f3f237051931ea5f2c36e3309e9db13658dbde2df1d',
          script: '',
          index: 0,
          prev_out: {
